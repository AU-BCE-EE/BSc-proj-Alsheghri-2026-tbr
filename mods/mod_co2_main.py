...
def gpm3_to_molpm3(c, M):
    """
    Function to convert g/m3 to mol/m3
    """
    return c / M

def molpm3_to_gpm3(c, M):
    """
    Function to convert from mol/m3 to g/m3
    """
    return c * M

# Rates function, a function defining the governing equations
# for CO2 absorption in aqueous solution of NaOH
...
def rates(t, n, 
          ...
          temp, henry, pres, 
          ssa, dens_l, por_l, por_g, cf = 1.0, counter = True, recirc = False, enh_method = 'PFO',
          constant_res_pH = False):
    
    # interpolation function for time variable input:
    ...
    # Use the interpolation function for the time-variable input 
    cgin      = interpolation(t, cgin)
    clin_co2  = interpolation(t, clin_co2)
    clin_TOTC = interpolation(t, clin_TOTC)

    # reaction rate for the reverse reaction HCO3- -> OH- + CO2
    k4 = k3/K4
    ...
    ncr     = n[(3 * nc) : (3 * nc) + 2]    # reservoir cells (1 for CO2 and 1 for TOTC)

    # The reservoir should handle 2 species separately (CO2 and TOTC)
    ncr_co2  = ncr[0]
    ncr_TOTC = ncr[1]

    # concentrations (mol/m3) 

    ccg    = ncg / vol_gas                    # CO2 in the gas phase     mol/m3
    ...
    # for integrating the speciation model we create empty arrays to store the results 
    c_h2co3 = np.zeros(nc)
    c_hco3  = np.zeros(nc)
    c_co3   = np.zeros(nc)
    c_oh    = np.zeros(nc)
    ...
    # for each TOTC value we now evaluate the speciation model and store the results
    for i in range(nc):
    ...
    # vant hoff equation for calculating new henry constant
    kh       = henry[0] * np.exp(henry[1] * (1/TK - 1/298.15)) # mol/kg-bar as liq:gas 
    ...
    if isinstance(Kga, str) and Kga.lower() =='onda':
        ...
        # characteristic packing length (m) (https://beckassets.blob.core.windows.net/product/readingsample/10075882/9783642262067_excerpt_001.pdf)
        dp     = 6 * (1 - (por_g+por_l)) / ssa  
        ...
        dens_g = pres * mw_g / (R * TK)               # density of gas kg/m3
        visc_g = 9.1E-8 * TK - 1.16E-5                # empirical relation for gas viscosity vs TK
        visc_l = -2.55E-5 * TK + 8.51E-3              # liquid viscosity 

        # Dimensionless numbers 
        Re = dens_l * v_l / (ssa * visc_l)            # Reynold number
        Fr = v_l * v_l * ssa / g                      # Froude number
        We = v_l * v_l * dens_l / (sigm_l * ssa)      # Weber number

        # effective wetted interfacial area 
        ae = ssa * (1.0-np.exp(-1.45 * (sigm_c / sigm_l)**0.75 *
                              Re**0.1 * Fr**-0.05 * We**0.2))    # m2/m3
     
        # gas phase mass transfer coefficient
        kg = dp_emp * (v_g * dens_g / (ssa * visc_g))**0.7 \
          * (visc_g / (dens_g * Dg))**(1 / 3) * (ssa * dp)**-2 * ssa * Dg  # m/s
        
        # liquid phase mass transfer coefficient
        kl = 0.0051 * (v_l * dens_l / (ae * visc_l))**(2/3) * (visc_l / (dens_l * Dliq))**(-0.5) \
            * (ssa * dp)**0.4 * (dens_l / (visc_l * g))**(-1/3)            # m/s
        
        # Enhancement factor function, the effect of chemical reaction on mass transfer
        ...
        Rtot = 1 / (kg * ae) + Kaw / (kl * ae * E) # reference p181 in Seader et al book (resistance in serie two film) (s)
        Kga  = 1 / Rtot * cf # overall mass transfer coefficient  (1/s)

    # model the reservoir as a cstr with two ODEs. One for CO2 and one for TOTC
    dmcr = np.zeros(2)   

    if recirc:
        if counter:
            oi = 0  # outlet index depends on flow direction
        else: 
            oi = nc - 1

        if v_res > 0:
            cr_co2  = ncr_co2 / v_res  # CO2 concentration in the reservoir mol/m3
            cr_TOTC = ncr_TOTC / v_res # TOTC concentration in the reservoir mol/m3
            
            # if we want to have a constant pH in the reservoir even though we have recirculation
            if constant_res_pH:
                ncr_TOTC = 0  
                cr_TOTC  = 0
            
            # speciation in the reservoir
            ...
            rxnr_co2 = (r1_res + r2_res) # reactions affecting CO2 

            # TOTC in the reservoir
            clin_TOTC = ncr_TOTC/v_res    # the dirichlet boundary condition ....
            rxnr_TOTC = (r1_res + r2_res) # reactions affecting TOTC
    ...
    if not counter:
        cvec_co2 = np.insert(c_co2, 0, clin_co2)     # insert the boundary condition in the inlet according to flow direction
    else:
        v_l_eff = - v_l
...
def tfmod(L, por_g, por_l, v_g, v_l, nc, cg0, cl_co20, cl_TOTC0, cgin, ex_oh, 
          ...
          pres = 1., ssa = 1100, cf = 1.0, typ = 'TBD', counter = True, recirc = False, enh_method = 'PFO',
          constant_res_pH = False):
    ...
    cf: float
        Correction factor for overall mass transfer coefficient
    v_g : float
    ...
    ex_oh : float
        Excess OH⁻/Na+ concentration (mol/m³)
    clin_co2 : float
    ...
    x    = x[1:(nc + 1)] - dx / 2     # Center position
    ...
    out = solve_ivp(rates, [0, max(times)], y0 = y0, 
                    ...
                    args = (v_g, v_l, cgin, clin_co2, clin_TOTC,
                            vol_gas, vol_liq, vol_tot,
                            k1, k2, k3, K4, K_hco3, K_co3, KW, ex_oh, Kga, v_res,
                            temp, henry, pres, ssa, dens_l, por_l, por_g, cf, counter, recirc,
    ...
    return {'gas_conc'    : molpm3_to_gpm3(ccgt,M_co2),
            ...
            'time'         : times,
            'inputs'       : args_in, 
            'pars'         : {'gas_rt': rt_gas, 'liq_rt': rt_liq, 'Kga': Kga,\
                               'Kaw': Kaw, 'ae':ae,'kg':kg,'kl':kl}}