...
# Parameters sweep to find optimal pH for the given flows

```{python}
pH_list = [13,13.2,13.5,13.7,13.9]
for pH in pH_list:
    results_biogas, cgin_biogas = modelrun(frac_co2 = 0.45,
                                    v_g = 0.5, 
                                    v_l =  1e-3,
...
This shows that even with pH = 13.9 we would not get sufficient removal, we need to change the flow velocities. 

## Lets try to lower the gas velocity 
```{python}
pH_list = [13,13.2,13.5,13.7,13.9]
for pH in pH_list:
    results_biogas, cgin_biogas = modelrun(frac_co2 = 0.45,
...
## lets raise the liquid flow velocity a little bit
...
So now we have a method to test different parameters :)
